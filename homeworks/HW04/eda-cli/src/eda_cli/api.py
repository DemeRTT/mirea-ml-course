...
# ---------- HW04: собственный эндпоинт, завязанный на эвристики HW03 ----------


class QualityFlagsResponse(BaseModel):
    """Полный набор булевых флагов качества (включая флаги из HW03)."""

    flags: dict[str, bool] = Field(
        ..., description="Словарь булевых флагов качества данных (ключ -> True/False)"
    )
...
@app.post(
    ...
    tags=["quality"],
    summary="Полный набор флагов качества по CSV (включая флаги из HW03)",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    print(
        ...
        f"n_rows={df.shape[0]} n_cols={df.shape[1]} n_flags={len(flags_bool)} "
        f"latency_ms={latency_ms:.1f} ms"
    )
